...
Masina getMasinaByID(Nod* radacina, int id) {
	...
	if (radacina) {
		...
		if (radacina->masina.id == id) {
			...
			strcpy_s(m.model,
				strlen(radacina->masina.model) + 1,
				radacina->masina.model);

			m.numeSofer = malloc(strlen(radacina->masina.numeSofer) + 1);
			strcpy_s(m.numeSofer,
				strlen(radacina->masina.numeSofer) + 1,
				radacina->masina.numeSofer);
...
float calculeazaPretTotal(Nod* radacina) {
	if (radacina == NULL)
		...
		float pretTotalStanga = calculeazaPretTotal(radacina->stanga);
		float pretTotalDreapta = calculeazaPretTotal(radacina->dreapta);
		return radacina->masina.pret + pretTotalStanga + pretTotalDreapta;
	}
}
...
int main() {
	...
	int n = determinaNumarNoduri(arbore);
	printf("\n%d", n);
	printf("\n%d", calculeazaInaltimeArbore(arbore));
	printf("\n%.2f", calculeazaPretTotal(arbore));
